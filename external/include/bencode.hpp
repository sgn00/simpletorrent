...
class map_proxy {
 public:
  ...
  template <typename K>
  mapped_type &at(K &&k) {
    return proxy_->at(std::forward<K>(k));
  }
  template <typename K>
...
struct variant_traits<std::variant> {
  ...
  inline static decltype(auto) visit(Visitor &&visitor,
                                     Variants &&...variants) {
...
using data =
    basic_data<std::variant, long long, std::string, std::vector, map_proxy>;
using data_view = basic_data<std::variant, long long, std::string_view,
                             std::vector, map_proxy>;

#ifdef BENCODE_HAS_BOOST
using boost_data =
    basic_data<boost::variant, long long, std::string, std::vector, map_proxy>;
using boost_data_view = basic_data<boost::variant, long long, std::string_view,
                                   std::vector, map_proxy>;
...
enum eof_behavior { check_eof, no_check_eof };
...
class decode_error : public std::runtime_error {
 public:
  ...
  [[noreturn]] void rethrow_nested() const {
    if (nested_) std::rethrow_exception(nested_);
    std::terminate();
  }

  std::exception_ptr nested_ptr() const noexcept { return nested_; }
...
template <typename Integer>
inline void check_over_underflow(Integer value, Integer digit, Integer sgn) {
  if (sgn == 1)
...
inline Integer decode_digits(Iter &begin, Iter end,
                             [[maybe_unused]] Integer sgn = 1) {
  assert(sgn == 1 ||
         (std::is_signed_v<Integer> && std::make_signed_t<Integer>(sgn) == -1));
  ...
  for (int i = 0; i != std::numeric_limits<Integer>::digits10; i++) {
    if (begin == end) throw end_of_input_error();
    if (!std::isdigit(*begin)) return value;
...
Integer decode_int(Iter &begin, Iter end) {
  ...
  if (*begin != 'e') throw syntax_error("expected 'e' token");
...
String decode_str(Iter &begin, Iter end) {
  ...
  if (begin == end) throw end_of_input_error();
  if (*begin != ':') throw syntax_error("expected ':' token");
  ++begin;
...
Data basic_decode(Iter &begin, Iter end) {
  ...
  auto store = [&result, &state, &dict_key](auto &&thing) -> Data * {
    if (state.empty()) {
      ...
      if (!i.second) {
        throw syntax_error("duplicated key in dict: " +
                           std::string(i.first->first));
      }
...
Data basic_decode(std::istream &s, eof_behavior e = check_eof) {
  ...
  if (e == check_eof && begin == end) s.setstate(std::ios_base::eofbit);
  return result;
}
...
class list_encoder {
 public:
  inline list_encoder(std::ostream &os) : os(os) { os.put('l'); }

  inline ~list_encoder() { os.put('e'); }
...
class dict_encoder {
 public:
  inline dict_encoder(std::ostream &os) : os(os) { os.put('d'); }

  inline ~dict_encoder() { os.put('e'); }
...
void write_integer(std::ostream &os, T value) {
  ...
  if (r.ec != std::errc()) throw std::system_error(std::make_error_code(r.ec));
...
template <typename T>
inline dict_encoder &dict_encoder::add(const string_view &key, T &&value) {
  encode(os, key);
  encode(os, std::forward<T>(value));
  return *this;
}
