...
TEST_CASE(
  ...
  // peer 1 arrives but cannot add to buffer, and cannot be assigned piece 0, so
  // it gets no block requests
  std::vector<uint8_t> bitfield_0{1, 0, 1, 0, 1};
...
TEST_CASE("PieceManager add_block complete one piece", "[PieceManager]") {
  ...
  std::string normal_hash = checksum.final();
  {
    PieceManager pm({hex_decode(normal_hash)}, piece_length, block_length,
                    total_length, output_file, buffer_size);
